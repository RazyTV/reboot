...
client.on("message", async message => {
  ...
  if(command === "say") {
    ...
    console.log(`${message.author.tag} used say command. Args: ${sayMessage}`)
  ...
  if(command === "clean") {
    ...
    message.channel.bulkDelete(fetched)
      ...
      console.log(`${message.author.tag} used Clean command. Args: ${deleteCount}`)
  }
  ...
  if(command === "seta") {
    ...
    console.log(`${message.author.tag} used seta command. Args: ${setActivity1}`)
  }

  if(command === "mw") {
    console.log(`${message.author.tag} used mw command.`)
    ...
    if(command === "whati") {
      console.log(`${message.author.tag} used whati command.`)
    if (message.member.voiceChannel) {
		var voice = message.member.voiceChannel
      message.member.voiceChannel.join()
        .then(connection => {
   ...
   if(command === "hbfs") {
     console.log(`${message.author.tag} used hbfs command.`)
    if (message.member.voiceChannel) {
		var voice = message.member.voiceChannel
      message.member.voiceChannel.join()
        .then(connection => {
    ...
    if(command === "ree") {
      console.log(`${message.author.tag} used ree command.`)
    if (message.member.voiceChannel) {
		var voice = message.member.voiceChannel
      message.member.voiceChannel.join()
        .then(connection => {
    ...
    if(command === "crab") {
      console.log(`${message.author.tag} used crab command.`)
    if (message.member.voiceChannel) {
		var voice = message.member.voiceChannel
      message.member.voiceChannel.join()
        .then(connection => {
  ...
  console.log(`${message.author.tag} used vip command. User: ${rMember}`)
  ...
  try{
    ...
    if(command === "daily") {
     ...
     console.log(`${message.author.tag} used daily command.`)
...
function cool(obj) {
    ...
    if(command === "daily2") {
     ...
     .then(res=>cool(res[1].img))
      console.log(`${message.author.tag} used daily2 command.`)
...
if(command === "avatar") {
 ...
 console.log(`${message.author.tag} used dank command.`)
	}
...
if(command === "timer") {
 ...
 console.log(`${message.author.tag} used timer command. args ${time}`)
...
const bitly = new BitlyClient('16c97a8d05eada1026f9c258cc1b59c37d4d403f', {});
 let text = args.join(" ");
 message.delete().catch(O_o=>{});
  console.log(`${message.author.tag} used short command. URL: ${text}`)
...
if(command === "bot") {
	let bicon = client.user.displayAvatarURL;
	let aembed = new Discord.RichEmbed()
      .setDescription("**reboot info**")
      .setColor("#00C8AF")
      .setImage(client.user.avatarURL)
      .addField("Bot version", "`2.5.1`")
      .addField("Created by:", "`RayzTV`")
      .addField("Running on:", "`Ubuntu Server 18.3`")
...
if(!message.member.roles.some(r=>["Creator", "Moderator"].includes(r.name)) )
  ...
  if(!message.member.roles.some(r=>["Creator", "Moderator"].includes(r.name)) )
      return message.reply("Sorry, you don't have permissions to use this!");
      var mess = message.channel
       resetBot(mess)
          }
  function resetBot(channel) {
     console.log(`Incoming restart!`)
